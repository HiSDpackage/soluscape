...
- For non-gradient systems: Direct specification of F in ? = -F(x)  
...
**Effect**  
Terminates search if ��x - x?��? > SearchArea  
...
### `PrintStepInterval` (Optional)  
**Description**  
Iteration frequency for console output when `WhetherReport=True`.  
...
**Note**  
Actual probes = `2 �� PerturbationCount` (bidirectional)  
