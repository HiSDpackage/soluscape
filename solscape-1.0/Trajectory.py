...
def DrawSearchTrajectory(instance, **kwargs):
    ...
    if ("DetailedTraj" not in kwargs) or (
        not isinstance(kwargs["DetailedTraj"], bool)
    ):
    ...
    kwargs.setdefault("TrajectorySet",
                      {"linewidth": 0.4, "linestyle": "-", "color": "blue", "label": "Search Trajectory"})
    kwargs["TrajectorySetFirst"] = copy.deepcopy(kwargs["TrajectorySet"])
    temp = kwargs["TrajectorySet"].pop("label", None)
    kwargs.setdefault("1DFunctionDraw",
                      {"linewidth": 2, "linestyle": "-", "color": "red", "label": "Function Curve"})

    # Saddle point settings
    colors = list(mcolors.CSS4_COLORS.keys())

    kwargs.setdefault("SaddlePointSet", [
        {"marker": "o", "color": colors[20 * i + 20], "label": f"Index {i} Saddle Point"}
    ...
    # Grid settings
    kwargs.setdefault("GridSet", {"visible": True, "linestyle": "--", "linewidth": 0.1, "color": "gray"})
    ...
    if instance.Dim > 2 or isfunction(kwargs["Projection"]):
        ...
        if kwargs["Projection"] is None or not isfunction(kwargs["Projection"]):
            ...
            for i in range(len(instance.SaddleList)):
                if WhetherSaddlePointDraw[instance.SaddleList[i][2]]:
                    projectpoint = kwargs["Projection"](instance.SaddleList[i][1].reshape(1, instance.Dim))
                    plt.scatter(
                        projectpoint[0, 0],
                        projectpoint[0, 1],
            ...
            for i in range(len(instance.DetailRecord)):
                if instance.DetailRecord[i][1] == -1 or (instance.SaddleList[instance.DetailRecord[i][1]][2] <=
                ...
                if kwargs["DetailedTraj"]:
                    ...
                    if WhetherTrajectoryDraw:
                        endpoint = kwargs["Projection"](instance.SaddleList[instance.DetailRecord[i][0]][1].reshape(1,instance.Dim))
                        beginpoint = kwargs["Projection"](instance.SaddleList[instance.DetailRecord[i][1]][1].reshape(1, instance.Dim))
                        x_values = [
                            beginpoint[0, 0],
                            endpoint[0, 0],
                        ]
                        ...
                        endpoint = kwargs["Projection"](
                            instance.SaddleList[instance.DetailRecord[i][0]][1].reshape(1, instance.Dim))
                        beginpoint = kwargs["Projection"](
                            instance.SaddleList[instance.DetailRecord[i][1]][1].reshape(1, instance.Dim))
                        x_values = [
                            beginpoint[0, 0],
                            endpoint[0, 0],
                        ]
        ...
        for i in range(len(instance.SaddleList)):
            if WhetherSaddlePointDraw[instance.SaddleList[i][2]]:
                ...
                WhetherSaddlePointDraw[instance.SaddleList[i][2]] = True
        for i in range(len(instance.DetailRecord)):
            if instance.DetailRecord[i][1]==-1 or (instance.SaddleList[instance.DetailRecord[i][1]][2] <= instance.SaddleList[instance.DetailRecord[i][0]][2]):
                continue
            if kwargs["DetailedTraj"]:
                temprecordlist = copy.deepcopy(instance.DetailRecord[i][2])
                temprecordlist = np.append(
