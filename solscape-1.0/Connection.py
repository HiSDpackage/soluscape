...
def DrawLandscapeConnection(instance, **kwargs):
    ...
    kwargs.setdefault("TrajectorySet",
                      {"width": 0.4, "style": "solid", "edge_color": "blue", "label": "Search Trajectory"})
    cmap = Pastel1
    colors = [mcolors.rgb2hex(cmap(i % cmap.N)) for i in range(instance.MaxIndex + 1)]
    kwargs.setdefault("SaddlePointSet", [
        {"node_shape": "o", "node_color": colors[i], "label": f"Index {i} Saddle Point"}
    ...
    nx.draw_networkx_labels(
        G, pos, labels=labels, font_size=math.ceil(0.45 *math.sqrt(math.ceil(3 * 400 / maxn)+7)), font_color="black"
    )
