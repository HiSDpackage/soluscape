...
def HiSDInitialization(instance):
    ...
    if instance.InitialEigenVectors is not None:
        v = np.array(copy.deepcopy(instance.InitialEigenVectors))  # Use provided eigenvectors
    else:
...
def HiSDIteration(instance):
    ...
    if j == instance.MaxIter:
        print("[WARNING] Iteration not converged: Maximum iterations reached without convergence. Skipping to next search.")
        instance.flag = False
        instance.DataBoundary = boundarytempsave
        return
    negnum, zeronum, posnum, negvecs = FindIndex(instance, x)
    if zeronum != 0:
        print(f"[WARNING] Degenerate saddle point detected under precision tol={instance.PrecisionTol}: Hessian matrix may contain zero eigenvalue(s).")
        print(f"Eigenvalue spectrum: negative={negnum}, zero={zeronum}, positive={posnum}. ")
    else:
        print(
