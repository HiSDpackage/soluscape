...
def HiSDCheckParam(instance, **kwargs):
    ...
    if "InitialPoint" not in kwargs:
        raise ValueError("Missing required parameter: `InitialPoint` must be specified.")
    else:
        ...
        if instance.InitialPoint.ndim > 2 or (
            ...
            raise ValueError(
                f"Parameter `InitialPoint` requires 1D array."
            )
        else:
            ...
            if "Dim" in kwargs:
                ...
                print(f"[Config Sync] `Dim` parameter auto-adjusted to {instance.Dim} based on `InitialPoint` dimensionality.")
    ...
    if "AutoDiff" not in kwargs:
        if "Grad" not in kwargs:
            ...
            if "EnergyFunction" in kwargs:
                ...
                print(f"Using `EnergyFunction` instead of `Grad` - enabling auto-differentiation mode.")
    ...
    if "GradientSystem" in kwargs:
        ...
        print("Parameter 'GradientSystem' not provided. Enabling automatic symmetry detection.")
    ...
    if "SymmetryCheck" in kwargs:
        ...
        print("Parameter 'SymmetryCheck' not provided. Defaulting to True with automatic detection.")
        instance.SymmetryCheck = True
    ...
    if not instance.GradSym:
        ...
        if instance.EigenMethod == "lobpcg":
            print("'lobpcg' incompatible with non-gradient systems. Reverting to 'power' method.")
        ...
        print("Gradient system detected. Activating HiOSD algorithm.")
        if instance.EigenMethod == "power":
            print("'power' incompatible with gradient systems. Reverting to 'lobpcg' method.")
...
def auto_checking_parameter(instance, param_name, kwargs, param_value):
    ...
    if param_name not in kwargs:
        print(f"Parameter `{param_name}` not specified - using default value {param_value}.")
        return param_value
    else:
        ...
        if param_name == "PrecisionTol":
            if isinstance(kwargs["PrecisionTol"], float) and kwargs["PrecisionTol"] >= 0.0:
        ...
        if param_name == "EigenMaxIter":
            if (
                isinstance(kwargs["EigenMaxIter"], int)
                and kwargs["EigenMaxIter"] > 0
            ):
        ...
        if param_name == "HessianDimerLength":
            if isinstance(kwargs["HessianDimerLength"], float) and kwargs["HessianDimerLength"] > 0:
        ...
        if param_name == "EigenStepSize":
            if isinstance(kwargs["EigenStepSize"], float) and kwargs["EigenStepSize"] > 0:
